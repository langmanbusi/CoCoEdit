...
class EditScore_0_5:
    def __init__(
        ...
        self.eval_prompt = vie_prompts.SCORE_LOGIT
    def evaluate(self, image_prompts, text_prompt):
        if not isinstance(image_prompts, list):
            image_prompts = [image_prompts]
        ...
        eval_prompt = self.eval_prompt.format(prompt=text_prompt)
        eval_prompt_final = self.model.prepare_input(image_prompts, eval_prompt)

        def parse_score(s, score_min=0.0, score_max=5.0):
            """从模型输出中解析一个 [0,5] 的分数，返回 float 或 None。"""
            if s is None:
                return None
        ...
        for i in range(self.num_pass):
            tries, max_tries = 0, 2
            score_val, last_raw = None, None
            while score_val is None and tries <= max_tries:
                tries += 1
                last_raw = self.model.inference(eval_prompt_final, seed=self.seed + i)
                score_val = parse_score(last_raw, 0.0, 5.0)
            if score_val is None:
                score_val = 2.5  # 解析失败时的保底
            scores.append(score_val)

        # 最终分数（仅返回一个数）
        final_score_0_5 = float(np.mean(scores))

        # 选一种返回尺度（按需取消注释）
        # return final_score_0_5                  # 0..5（推荐，与你的提示一致）
        # return final_score_0_5 / 5.0          # 0..1
        return final_score_0_5          # 0..10（若需与旧管线兼容）
